...
CGRect MPITextCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
...
CGFloat MPITextOnePixel() {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        onePixel = 1.0 / MPITextScreenScale();
    });
    return onePixel;
}

CGAffineTransform MPITextCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from mpi_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from mpi_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from mpi_convertPoint:before[2] toViewOrWindow:to];
    
    return MPITextCGAffineTransformGetFromPoints(before, after);
}

CGAffineTransform MPITextCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
